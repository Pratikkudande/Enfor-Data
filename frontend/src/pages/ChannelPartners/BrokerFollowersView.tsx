...
import { Network, Search, MapPin, Building2, Home, Phone, Loader2, Users } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-gray-600 text-sm">Brokers who follow you.</p>
        </div>
      </div>

      <div className="flex items-center gap-2 text-sm text-gray-500">
        <Users className="h-4 w-4" /> {brokers.length} follower{brokers.length !== 1 ? 's' : ''}
      </div>
      ...
      {!loading && !error && filtered.length === 0 && (
        <div className="text-center py-16 bg-white rounded-xl border border-gray-100">
          <Network className="h-8 w-8 text-gray-300 mx-auto mb-3" />
          <p className="text-gray-500">No brokers are following you yet.</p>
        </div>
      )}
              ...
              <div key={b.id} className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow border border-gray-100 overflow-hidden">
                ...
                <div className="p-6">
                  ...
                  <div className="space-y-1.5 text-sm text-gray-600">
                    ...
                    {b.whatsapp_number && (
                      <a href={`tel:${b.whatsapp_number}`} className="flex items-center gap-2 hover:text-blue-600">
                        <Phone className="h-4 w-4 text-gray-400" /> {b.whatsapp_number}
                      </a>
                    )}
                  </div>
                </div>
