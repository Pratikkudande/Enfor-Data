...
import { createDLTTemplate, CreateDLTTemplateRequest, getAvailableHeadersByType, SMSHeader } from '../../../services/smsMarketingApi';
import { useAuth } from '../../../context/AuthContext';

interface AddDLTTemplateModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

const AddDLTTemplateModal: React.FC<AddDLTTemplateModalProps> = ({ isOpen, onClose, onSuccess }) => {
  const { user } = useAuth();
  const isBroker = user?.role === 'broker' || user?.role === 'channel_partner';

  const [formData, setFormData] = useState<CreateDLTTemplateRequest>({
    header: '',
    template_id: '',
    template_name: '',
  ...
  // Load headers when template type changes
  useEffect(() => {
  ...
  const handleChange = (field: keyof CreateDLTTemplateRequest, value: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      await createDLTTemplate(formData);
      alert('DLT Template added successfully!');
      onSuccess();
      onClose();
      // Reset form
      setFormData({
      ...
      alert('Failed to add template: ' + (error.response?.data?.message || error.message));
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto m-4">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900">Add DLT Template</h2>
              <p className="text-sm text-gray-600">Create a new SMS template</p>
            </div>
          </div>
          <button
            onClick={onClose}
...
export default AddDLTTemplateModal;
